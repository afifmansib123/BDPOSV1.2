...
const navigate = useNavigate(); 
  ...
  const [waiterName, setWaiterName] = useState(""); // To store waiter name
  
  const CloseOrder = async (id) => {
    if (!waiterName) {
      message.error("Please enter the waiter's name");
      return;
    }
  
    try {
      await fetch(process.env.REACT_APP_SERVER_URL + "/bills/update-bill", {
        method: "PUT",
        body: JSON.stringify({ _id: id, completed: true, waitertoclose : waiterName }), // Include waiterName in the request
  ...
  {/* Trigger to open the waiter name input modal */}
  <Button
    ...
    onClick={() => setIsWaiterModalOpen(true)} // Open waiter input modal
  >
    Close Order
  </Button>
  ...
  <Link to="/">
    <Button
      className="ml-5 bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white px-4 border border-blue-500 hover:border-transparent rounded"
      onClick={() => { AddtoOrder(customer._id) }}
    >
      Add Items
    </Button>
  </Link>
...
<Modal
  title="Waiter Closing the Order"
  open={isWaiterModalOpen}
  onOk={async () => {
    if (!waiterName) {
      message.error("Please enter the waiter's name");
      return; // Prevent the operation from proceeding if the waiter name is empty
    }

    try {
      await CloseOrder(customer._id); // Wait for CloseOrder to finish
      setIsWaiterModalOpen(false); // Close the waiter modal
      setIsModalOpen2(false);      // Close the main order modal
      navigate("/"); 
    } catch (error) {
      message.error("Failed to close the order. Please try again.");
    }
  }}
  onCancel={() => {
    setIsWaiterModalOpen(false); // Close waiter name modal
  }}
>
  <Input
    placeholder="Enter waiter's name"
    value={waiterName}
    onChange={(e) => setWaiterName(e.target.value)} // Update waiterName state
  />
</Modal>
